...
int main()
{
	double start_time = clock(); // Начальное время (для счета количества времени работы проги)
	...
	double end_time = clock(); // Конечное время (для счета количества времени работы проги)
	printf("Прошло времени с начала работы: %.3f msec\n", end_time - start_time); // Расчет количества времени затраченное на работу всей проги в msec
	system("PAUSE");
	return 0;
}
